...
- **🎨 主题生态** — 兼容原版 CFSM 全部主题（主题商店一键安装），面板后台还支持自定义 CSS / JS / 背景图；每台最多 24 个探测点，内置主题与商店 fork 可读 `probes[]`
...
| `NODE_OPTIONS` | Node 堆上限（默认按容器内存上限自动设置，一般无需配置；手动设置优先） | 自动（1/5，64–384MB） |

> 说明：Node（V8）默认按**宿主机物理内存**算堆上限、不看容器限制；小容器跑在大内存宿主机上，内存占用会缓慢变大。
> 本镜像已内置自动处理：按**容器内存上限的 1/5** 设置 Node 堆上限（限制在 64–384MB）；自己设了 `NODE_OPTIONS` 则以你的为准。
...
**Q：面板内存会慢慢变大？**
镜像会按容器内存上限自动设置 Node 堆上限（见「环境变量」），正常不会越跑越胖。
若观察到持续增长，先确认镜像已更新到最新版（旧版无此自动处理）。
