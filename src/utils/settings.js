...
export const DEFAULT_ONLINE_THRESHOLD_SECONDS = 300;
export const PUBLIC_HISTORY_HOUR_OPTIONS = [24, 48, 96, 168, 336, 720];
export const DEFAULT_PUBLIC_HISTORY_HOURS = 24;
export const NOTIFICATION_EVENT_EMOJI_KEYS = ['offline', 'recover', 'expire', 'alert', 'traffic', 'test'];
export const TRAFFIC_REPORT_TYPES = ['daily', 'weekly', 'monthly'];
...
export function normalizeOnlineThresholdSeconds(value) {
  if (value === undefined || value === null || value === '') return '';
  const seconds = Number(value);
  return Number.isFinite(seconds) && ONLINE_THRESHOLD_SECOND_OPTIONS.includes(seconds)
    ? String(seconds)
    : '';
}

export function resolveOnlineThresholdSeconds(value) {
  const normalized = normalizeOnlineThresholdSeconds(value);
  return normalized ? Number(normalized) : DEFAULT_ONLINE_THRESHOLD_SECONDS;
}

export function normalizePublicHistoryHours(value) {
  if (value === undefined || value === null || value === '') return '';
  const hours = Number(value);
  return Number.isFinite(hours) && PUBLIC_HISTORY_HOUR_OPTIONS.includes(hours)
    ? String(hours)
    : '';
}

// 未登录访客可查看的历史范围上限（默认为 24 小时，保持旧行为）
export function resolvePublicHistoryHours(value) {
  const normalized = normalizePublicHistoryHours(value);
  return normalized ? Number(normalized) : DEFAULT_PUBLIC_HISTORY_HOURS;
}
...
// 每事件 Emoji 映射：仅保留已知事件键，值去空白、最长 16 个字符；空值不落库
export function normalizeNotificationEventEmojis(value) {
  let source = value;
  ...
  const result = {};
  for (const key of NOTIFICATION_EVENT_EMOJI_KEYS) {
    const raw = source[key];
    const emoji = typeof raw === 'string' ? raw.trim().slice(0, 16) : '';
    if (emoji) result[key] = emoji;
  }
  return result;
}

// 自定义 JS 通知脚本：保持原文（不 trim/不改动），仅做类型兜底
export function normalizeNotificationCustomScript(value) {
  if (value === undefined || value === null) return '';
  return typeof value === 'string' ? value : String(value);
}

// 流量报告类型：接受数组或逗号分隔字符串；返回规范化 CSV（固定顺序；空 = 未配置）
export function normalizeTrafficReportTypes(value) {
  let source = value;
...
export function resolveTrafficReportTypes(settings = {}) {
  const normalized = normalizeTrafficReportTypes(settings?.traffic_report_types);
  if (normalized) return normalized.split(',');
  return normalizeBooleanSetting(settings?.traffic_report_enabled) === 'true'
    ? [...TRAFFIC_REPORT_TYPES]
    : [];
}
...
export async function loadSiteSettings(db, options = {}) {
  ...
  try {
    ...
    result.online_threshold_seconds = normalizeOnlineThresholdSeconds(result.online_threshold_seconds);
    result.public_history_hours = normalizePublicHistoryHours(result.public_history_hours);
    ...
    result.notification_custom_script = normalizeNotificationCustomScript(result.notification_custom_script);
    result.notification_event_emojis = normalizeNotificationEventEmojis(result.notification_event_emojis);
...
export async function saveSiteOptions(db, updates) {
  ...
  siteOptions.public_history_hours = normalizePublicHistoryHours(siteOptions.public_history_hours);
  ...
  siteOptions.notification_custom_script = normalizeNotificationCustomScript(siteOptions.notification_custom_script);
  siteOptions.notification_event_emojis = normalizeNotificationEventEmojis(siteOptions.notification_event_emojis);
